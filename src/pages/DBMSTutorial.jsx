...
const DBMSTutorial = () => {
    ...
    useEffect(() => {
        const fetchTopics = async () => {
            try {
                const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/domains/topics/corecs?subject=DBMS&lessonType=theory`);
                if (res.data && res.data.length > 0) {
                    setTopic(res.data[0]);
                }
                setLoading(false);
            } catch (e) {
                console.error('Error fetching DBMS data:', e);
                setLoading(false);
            }
        };
        fetchTopics();
    ...
    return (
        <div className="tutorial-page tp-theme-dbms">
            <div className="tp-hero">
                ...
                <div className="tp-hero-content">
                    ...
                    <h1>Master <span className="tp-gradient-text">Database Systems</span></h1>
                    <p>Relational databases, SQL Querying, Normalization, ACID properties, and Architecture.</p>
                </div>
            </div>
                ...
                <main className="tp-main">
                    <section id="tutorial" className="tp-section">
                        <div className="tp-section-header">
                            <h2 className="tp-section-title">DBMS & SQL Tutorial</h2>
                            <p className="tp-section-subtitle">A deep dive into structural data management and querying.</p>
                        </div>
                    ...
                    <section id="mcqs" className="tp-section">
                        <div className="tp-section-header">
                            <h2 className="tp-section-title">DBMS Quiz</h2>
                        ...
                        {!showMcqs ? (
                            <div className="tp-mcq-card text-center flex flex-col items-center gap-6 py-16">
                                ...
                                <div className="max-w-md">
                                    ...
                                    <p className="tp-text-muted mb-8">Challenge yourself with questions on Normalization, Joins, Indexing, and Transactions.</p>
                                    <button
                                        className="btn btn-primary px-10 py-4 rounded-full text-lg"
                                        onClick={() => setShowMcqs(true)}
                                        style={{ backgroundColor: '#818cf8', boxShadow: '0 4px 14px rgba(129, 140, 248, 0.4)' }}
                                    >
                                        Start DBMS Quiz
                                    </button>
                                </div>
