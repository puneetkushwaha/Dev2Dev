...
import { ChevronDown, Target, Clock, Activity, MessageSquare, BookOpen, Search, Star, Languages, Info, FileText, Loader2, Code2, Calculator, Scale, Hash, User } from 'lucide-react';
...
const VerbalAbility = () => {
    ...
    useEffect(() => {
        const fetchTopics = async () => {
            try {
                ...
                const qt = res.data.filter(t => t.topicGroup === 'Verbal Ability');
                ...
                setVerbalAbilityData(formatted);
    ...
    useEffect(() => {
        if (!verbalAbilityData.length) return;
        const handleScroll = () => {
            let current = verbalAbilityData[0]?.id;
            for (let section of verbalAbilityData) {
    ...
    if (loading) {
        return <Loader text="Assembling verbal modules..." />;
    }

    return (
        <div className="oops-page">
            <div className="oops-hero" style={{ background: 'linear-gradient(180deg, rgba(244, 63, 94, 0.05) 0%, var(--bg-primary) 100%)' }}>
                <div className="oops-animated-bg" style={{ background: 'radial-gradient(circle, rgba(244, 63, 94, 0.15) 0%, transparent 60%)' }}></div>
                <div className="hero-content text-center">
                    <div className="badge" style={{ color: '#f43f5e', background: 'rgba(244, 63, 94, 0.1)', borderColor: 'rgba(244, 63, 94, 0.2)' }}>
                        <Calculator size={16} /> Placement Preparation
                    </div>
                    <h1>Master <span>Verbal Ability</span></h1>
                    <p>Essential English grammar, vocabulary, and comprehension questions to crack technical interviews and placement exams.</p>
                </div>
            </div>
                ...
                <aside className="oops-sidebar custom-scrollbar">
                    <div className="sidebar-sticky">
                        ...
                        <nav className="sidebar-nav">
                            {verbalAbilityData.map(section => (
                                <button
                                    key={section.id}
                                    className={activeSection === section.id ? 'active va-active' : ''}
                        ...
                        <div className="sidebar-info-card" style={{ background: 'rgba(244, 63, 94, 0.05)', borderColor: 'rgba(244, 63, 94, 0.2)' }}>
                            <Info size={20} color="#f43f5e" />
                            <p>This guide features solved problems on frequently asked verbal ability topics.</p>
                        </div>
                    </div>
                </aside>
                                ...
                                {section.questions.map((mcq, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className="mcq-card">
                                            ...
                                            {showResult && (
                                                <div className="mcq-explanation mt-4 p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px dashed rgba(244, 63, 94, 0.3)' }}>
                                                    <div className="flex items-center gap-2 mb-2" style={{ color: '#fb7185', fontWeight: '600', fontSize: '0.9rem' }}>
            ...
            <style>{`
                /* Verbal Ability Styles (Rose Theme #f43f5e) */
                ...
                .hero-content h1 span { color: transparent; -webkit-background-clip: text; background-clip: text; background-image: linear-gradient(90deg, #fb7185, #f43f5e); }
                ...
                .sidebar-nav button.va-active { background: rgba(244, 63, 94, 0.1) !important; color: #fb7185 !important; border-color: rgba(244, 63, 94, 0.2) !important; font-weight: 600; }
                ...
                .mcq-card:hover { transform: translateY(-4px); border-color: #f43f5e; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.3), 0 0 15px rgba(244, 63, 94, 0.15); }
                ...
                .mcq-option:hover:not(.disabled) { background: rgba(244, 63, 94,0.05); border-color: #e11d48; color: #e11d48; }
                ...
                .mcq-option.va-selected { border-color: #f43f5e; background: rgba(244, 63, 94,0.05); }

                /* Explanation block & Theory Area Styling */
                .theory-container { background: rgba(244, 63, 94, 0.03); border: 1px solid rgba(244, 63, 94, 0.15); border-radius: var(--radius-lg, 16px); padding: 1.5rem; margin-bottom: 2rem; }
                .theory-container h4 { color: #fb7185; font-size: 1.1rem; margin-top: 0; margin-bottom: 1rem; font-weight: 700; display: flex; align-items: center; gap: 0.5rem; }
                ...
                .theory-container code { background: rgba(255,255,255,0.1); padding: 0.1rem 0.4rem; border-radius: 4px; font-family: monospace; font-size: 0.9em; color: #ffe4e6; }

                .qa-text { font-size: 0.95rem; color: rgba(255,255,255,0.7); line-height: 1.7; }
                .qa-text b { color: #fff; font-weight: 600; }
                .qa-text i { color: #fb7185; }
