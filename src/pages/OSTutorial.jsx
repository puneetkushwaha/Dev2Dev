...
const OSTutorial = () => {
    ...
    useEffect(() => {
        const fetchTopics = async () => {
            try {
                const res = await axios.get('http://localhost:5000/api/domains/topics/corecs?subject=OS&lessonType=theory');
    ...
    if (loading) {
        return (
            <div className="flex h-screen items-center justify-center bg-[#09090e] text-white">
                ...
                <span className="ml-3 font-semibold">Loading OS Tutorial...</span>
            </div>
        );
    }
