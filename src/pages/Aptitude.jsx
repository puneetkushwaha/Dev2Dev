...
const Aptitude = () => {
    ...
    useEffect(() => {
        const fetchTopics = async () => {
            try {
                ...
                setAptitudeData(formatted);
                if (formatted.length > 0) setActiveSection(formatted[0].id);
                setLoading(false);
            } catch (e) { console.error('Error fetching aptitude data:', e); setLoading(false); }
        };
        fetchTopics();
    }, []);

    useEffect(() => {
        if (!aptitudeData.length) return;
        const handleScroll = () => {
            let current = aptitudeData[0]?.id;
            for (let section of aptitudeData) {
    ...
    const getIcon = (iconName) => {
        switch (iconName) {
            ...
            case 'Clock': return <Clock size={14} className="inline mr-2 opacity-70" />;
            ...
            case 'Percent': return <Percent size={14} className="inline mr-2 opacity-70" />;
            case 'FlaskConical': return <FlaskConical size={14} className="inline mr-2 opacity-70" />;
            case 'Calculator': return <Calculator size={14} className="inline mr-2 opacity-70" />;
            ...
            case 'CircleDollarSign': return <CircleDollarSign size={14} className="inline mr-2 opacity-70" />;
    ...
    return (
        <div className="oops-page">
            <div className="oops-hero" style={{ background: 'linear-gradient(180deg, rgba(245, 158, 11, 0.05) 0%, var(--bg-primary) 100%)' }}>
                <div className="oops-animated-bg" style={{ background: 'radial-gradient(circle, rgba(245, 158, 11, 0.15) 0%, transparent 60%)' }}></div>
                <div className="hero-content text-center">
                    <div className="badge" style={{ color: '#f59e0b', background: 'rgba(245, 158, 11, 0.1)', borderColor: 'rgba(245, 158, 11, 0.2)' }}>
                        <Calculator size={16} /> Placement Preparation
                    </div>
                    <h1>Master <span>Quantitative Aptitude</span></h1>
                    <p>Essential mathematical problems and logical reasoning questions to crack technical interviews and placement exams.</p>
                </div>
            </div>
                ...
                <aside className="oops-sidebar custom-scrollbar">
                    <div className="sidebar-sticky">
                        ...
                        <nav className="sidebar-nav">
                            {aptitudeData.map(section => (
                                <button
                                    key={section.id}
                                    className={activeSection === section.id ? 'active apt-active' : ''}
                        ...
                        <div className="sidebar-info-card" style={{ background: 'rgba(245, 158, 11, 0.05)', borderColor: 'rgba(245, 158, 11, 0.2)' }}>
                            <Info size={20} color="#f59e0b" />
                            <p>This guide features solved problems on frequently asked aptitude topics.</p>
                        </div>
                    </div>
                </aside>
                                ...
                                {section.questions.map((mcq, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className="mcq-card">
                                            ...
                                            <div className="mcq-options">
                                                {mcq.options.map((opt, oIdx) => {
                                                    ...
                                                    if (showResult) {
                                                        ...
                                                        optClass += " selected apt-selected";
                                                    }
                                            ...
                                            {showResult && (
                                                <div className="mcq-explanation mt-4 p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px dashed rgba(245, 158, 11, 0.3)' }}>
                                                    <div className="flex items-center gap-2 mb-2" style={{ color: '#fbbf24', fontWeight: '600', fontSize: '0.9rem' }}>
            ...
            <style>{`
                /* Aptitude Styles (Yellow/Orange Theme #f59e0b) */
                ...
                .hero-content h1 span { color: transparent; -webkit-background-clip: text; background-clip: text; background-image: linear-gradient(90deg, #fbbf24, #f59e0b); }
                ...
                .sidebar-nav button.apt-active { background: rgba(245, 158, 11, 0.1) !important; color: #fbbf24 !important; border-color: rgba(245, 158, 11, 0.2) !important; font-weight: 600; }
                ...
                .mcq-card:hover { transform: translateY(-4px); border-color: #f59e0b; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.3), 0 0 15px rgba(245, 158, 11, 0.15); }
                ...
                .mcq-option:hover:not(.disabled) { background: rgba(245, 158, 11,0.05); border-color: #d97706; color: #d97706; }
                ...
                .mcq-option.apt-selected { border-color: #f59e0b; background: rgba(245, 158, 11,0.05); }

                /* Explanation block & Theory Area Styling */
                .theory-container { background: rgba(245, 158, 11, 0.03); border: 1px solid rgba(245, 158, 11, 0.15); border-radius: var(--radius-lg, 16px); padding: 1.5rem; margin-bottom: 2rem; }
                .theory-container h4 { color: #fbbf24; font-size: 1.1rem; margin-top: 0; margin-bottom: 1rem; font-weight: 700; display: flex; align-items: center; gap: 0.5rem; }
                ...
                .theory-container code { background: rgba(255,255,255,0.1); padding: 0.1rem 0.4rem; border-radius: 4px; font-family: monospace; font-size: 0.9em; color: #fde68a; }

                .qa-text { font-size: 0.95rem; color: rgba(255,255,255,0.7); line-height: 1.7; }
                .qa-text b { color: #fff; font-weight: 600; }
                .qa-text i { color: #fbbf24; }
                .qa-text p { margin: 0.5rem 0; }
                .qa-text ul, .qa-text ol { margin: 0.5rem 0; padding-left: 1.5rem; }
                .qa-text li { margin-bottom: 0.25rem; }
                .qa-text code { background: rgba(255,255,255,0.1); padding: 0.1rem 0.4rem; border-radius: 4px; font-family: monospace; font-size: 0.9em; color: #fde68a; }
