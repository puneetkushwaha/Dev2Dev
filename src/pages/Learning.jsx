...
const Learning = () => {
        ...
        const fetchDomainTopics = async () => {
            ...
            try {
                ...
                const topicsRes = await axios.get(
                    `http://localhost:5000/api/domains/topics/by-domain/${domain._id}`
                );
    ...
    const handleStartTopic = async (topicOrTitle, domainOverride = null) => {
        ...
        if (!topicId) {
            try {
                ...
                if (domainDoc) {
                    const tRes = await axios.get(`http://localhost:5000/api/domains/topics/${domainDoc._id}`);
        ...
        try {
            ...
            if (topicId) {
                const res = await axios.get(`http://localhost:5000/api/domains/topics/detail/${String(topicId)}`);
                const t = res.data;
            ...
            const aiRes = await fetch('http://localhost:8000/generate_lesson', {
                method: 'POST',
            ...
            try {
                await axios.post('http://localhost:5000/api/domains/topics/cache-content', {
                    domainName: domain,
    ...
    const saveProgress = async (topic) => {
        const token = localStorage.getItem('token');
        try {
            await fetch('http://localhost:5000/api/users/complete-topic', {
                method: 'POST',
                headers: {
