...
const ProblemView = () => {
    ...
    useEffect(() => {
        const fetchProblem = async () => {
            try {
                const res = await axios.get(`http://localhost:5000/api/users/problems/${id}`, {
    ...
    const handleSubmit = async (isRun = false) => {
        ...
        try {
            console.log(`🚀 Sending submission to: http://localhost:5000/api/users/submit-exam`);
            console.log(`📦 Payload:`, {
            ...
            const res = await axios.post('http://localhost:5000/api/users/submit-exam', {
                examName: problem.title,
                language: selectedLang.id,
